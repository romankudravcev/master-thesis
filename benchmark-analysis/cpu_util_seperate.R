# ============================================================
# R-Skript: CPU Usage Plots - 2x4 Grid mit dynamischer Legende
# ============================================================
...
FIXED_CPU_MAX <- 50    # Alle Y-Achsen gehen von 0 bis 50%
...
process_file <- function(filepath) {
  ...
  tryCatch({
    ...
    required_cols <- c("timestamp", "cluster_cpu_usage")
    ...
    df <- df %>% 
      mutate(time_index = as.numeric(difftime(timestamp, start_time, units = "secs"))) %>%
      arrange(time_index) %>%
      filter(time_index < 600, !is.na(cluster_cpu_usage)) %>%
      select(time_index, cluster_cpu_usage)
...
process_group <- function(files, label) {
  ...
  averaged <- combined %>% 
    group_by(time_index) %>% 
    summarise(cluster_cpu_usage = mean(cluster_cpu_usage, na.rm = TRUE), .groups = "drop") %>%
...
create_subplot <- function(db, db_type, conn_tool, cluster_type) {
  ...
  if(is.null(data) || nrow(data) == 0) {
    ggplot() + 
      annotate("text", x = FIXED_TIME_MAX/2, y = FIXED_CPU_MAX/2, 
               label = "No data", size = 3) +
      labs(title = title, x = "Time (s)", y = "CPU (%)") +
      xlim(0, FIXED_TIME_MAX) +
      ylim(0, FIXED_CPU_MAX) +
      theme_minimal() +
      theme(
    ...
    ggplot(data, aes(x = time_index, y = cluster_cpu_usage, color = migration_type)) +
      geom_line(linewidth = 0.8) +
      scale_color_manual(values = dynamic_colors, name = "Type") +
      labs(x = "Time (s)", y = "CPU (%)", title = title) +
      xlim(0, FIXED_TIME_MAX) +
      ylim(0, FIXED_CPU_MAX) +
      theme_minimal() +
      theme(
...
create_legend_plot <- function(conn_tool) {
  ...
  dummy_data <- data.frame(
    ...
    cluster_cpu_usage = rep(c(10, 20, 30), 3),  # Verschiedene Y-Werte
  ...
  legend_plot <- ggplot(dummy_data, aes(x = time_index, y = cluster_cpu_usage, color = migration_type)) +
...
cat("Verwende feste Achsenlimits: X = 0-", FIXED_TIME_MAX, "s, Y = 0-", FIXED_CPU_MAX, "%\n\n")
...
for(conn_tool in connectivity_tools) {
  ...
  # PDF Dateiname
  pdf_filename <- paste0("cpu_usage_analysis_", conn_tool, ".pdf")
...
cat("Alle Y-Achsen: 0 bis", FIXED_CPU_MAX, "% CPU Usage\n")
...
combined_pdf_path <- file.path(output_path, "cpu_usage_analysis_all_tools.pdf")
